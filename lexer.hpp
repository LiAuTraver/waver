    ...
    void print_parsed_data() {
        ...
        for (const auto &[module, vars] : module_variables) {
            ...
            for (const auto &[identifier, signal] : vars) {
                std::cout << "  " << identifier << ": " << signal.name << " (" << signal.width << " bits)" << std::endl;
            }
        }
...
private:
    std::unordered_map<std::string, std::unordered_map<std::string, Signal>> module_variables;
    ...
    void parse_variable(const std::string &line, const std::string &module_name) {
        ...
        int width;
        iss >> temp >> type >> width >> identifier >> name;
        module_variables[module_name][identifier] = {type, width, identifier, name};
    }
